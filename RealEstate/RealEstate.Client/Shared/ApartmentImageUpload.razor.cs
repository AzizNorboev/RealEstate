...
{
    public partial class ApartmentImageUpload
    {
        ...
        [Inject]
        public IHttpRepository<Apartment> ApartmentRepository { get; set; }
            ...
            foreach (var file in await FileReaderService.CreateReference(_input).EnumerateFilesAsync())
            {
                if (file != null)
                {
                    ...
                    using (var ms = await file.CreateMemoryStreamAsync(4 * 1024))
                    {
                        ...
                        ImgUrl = await ApartmentRepository.UploadImage(content);
